from spikelearn import SpikingNet, HomeostasisLayer, SecondOrderLayer, PlasticSynapse, LambdaSynapse, OneToOneSynapse, SynapseCircuitMultiwire, BaseSynapse, cell_7T1R, cell_1T1R, cell_7T1R_bp, SynapseCircuitMultiwire_bp
import numpy as np
from spikelearn.generators import Poisson
from spikelearn.trace import ConstantTrace, Trace, TraceTransform, CombinedTrace
import itertools
import math
import os
import re
...
for layer_desc in layers_desc:
    ...
    if options.synapse_type=='ideal':
        ...
        syn_input_exc=PlasticSynapse(N_inputs, N_excitatory, winit, prez, postz,
        ...
        # dep: e^30 per volt
        cell=cell_7T1R(winit, g1, 1e-9, 10e-9, fname, 0.7, incl_off=True, vg_on=0.9)
        cell.leak_cur=lambda v:(9.5055E-08*v*v - 1.9313E-08*v + 8.9247E-09)
        prez=ConstantTrace(N_inputs, 0)
        postz=ConstantTrace(N_excitatory, 0)
        ...
        syn_input_exc=SynapseCircuitMultiwire(cell, N_inputs, N_excitatory, prez, postz, 4, 3, [Ctpre, Csr, CspreL, CspreH], [Ctpost, CspostL, CspostH], [], ([prez, ('xe',min(1.08,cell.Vread+0.4)), prez, prez],[postz, postz, postz]), syn_type='exc')
    ...
    elif options.synapse_type=='conf_17':
        fname, g1, v1_p, v1_n, dv_ep, dv_en = args
        g1=float(g1)
        v1_p=float(v1_p)
        v1_n=float(v1_n)
        ...
        # dep: e^30 per volt
        cell=cell_7T1R(winit, g1, 1e-9, 10e-9, fname, 0.7, incl_off=True, vg_on=0.9)
        cell.leak_cur=lambda v:min(9.5055E-08*v*v - 1.9313E-08*v + 8.9247E-09, 1e-12)
        prez=ConstantTrace(N_inputs, 0)
        postz=ConstantTrace(N_excitatory, 0)
        Ctpre=N_excitatory*(26*1.96+210*2.4)*1e-18
        # CspreL=0
        ...
        #syn_input_exc=SynapseCircuit(cell, N_inputs, N_excitatory, trace_pre, trace_post, syn_type='exc', tracelim=1)
        syn_input_exc.W=winit
    elif options.synapse_type=='5T1R':
        fname, g1, v1_p, v1_n, dv_ep, dv_en = args
        g1=float(g1)
        del v1_p
        v1_n=float(v1_n)
        ...
        dv_en=float(dv_en)# 1/30
        # dep: e^30 per volt
        cell=cell_7T1R_bp(winit, g1, 1e-9, fname, 0.7, incl_off=True, vg_on=0.9)
        cell.leak_cur=lambda v:(7.63E-08*v*v + 5.73E-08*v + 4.01E-09)
        prez=ConstantTrace(N_inputs, 0)
        postz=ConstantTrace(N_excitatory, 0)
        vposterrp=postz
        vposterrn=postz
        ...
        syn_input_exc=SynapseCircuitMultiwire_bp(cell, N_inputs, N_excitatory, CombinedTrace(), CombinedTrace(), 4, 4, [CspreL, CspreH, CspreH, 0], [CspostL, 0, Ctpost, Ctpost], [], ([('xe',cell.vsdepL),('xe',cell.vsdepH),prez,prez],[postz,postz,vposterrp,vposterrn]), [([prez,prez,prez,prez],[vposterrpbinL,vposterrpbinH,vposterrp,vposterrn]),([prez,prez,prez,prez],[vposterrnbinL,vposterrnbinH,vposterrp,vposterrn])], syn_type='exc')
        syn_input_exc.W=winit
    elif options.synapse_type=='conf_15':
        fname, g1, v1_p, v1_n, dv_ep, dv_en = args
        g1=float(g1)
        del v1_p
        v1_n=float(v1_n)
        ...
        # dep: e^30 per volt
        cell=cell_7T1R_bp(winit, g1, 1e-9, fname, 0.7, incl_off=True, vg_on=0.9)
        cell.leak_cur=lambda v:min(7.63E-08*v*v + 5.73E-08*v + 4.01E-09, 1e-12)
        prez=ConstantTrace(N_inputs, 0)
        postz=ConstantTrace(N_excitatory, 0)
        vposterrp=postz
        vposterrn=postz
        ...
        # dep: e^30 per volt
        cell=cell_7T1R(winit, g1, 1e-9, 10e-9, fname, 0.7, incl_off=True, vg_on=0.9)
        cell.leak_cur=lambda v:(2e-10 if v>0.5 else 0)
        prez=ConstantTrace(N_inputs, 0)
        postz=ConstantTrace(N_excitatory, 0)
        Ctpre=N_excitatory*(26*1.96+210*2)*1e-18
        # CspreL=0
        # CspreH=N_excitatory*(26*1.96*3+210*2.4)*1e-18
        CspreL=N_excitatory*(26*1.96+210*2)*1e-18
        CspreH=N_excitatory*(26*1.96+210*2)*1e-18
        Csr=N_excitatory*(26*1.96+210*2)*1e-18
        Ctpost=N_inputs*(210*2.4)*1e-18
        # CspostL=0
        # CspostH=N_inputs*(26*1.96*2+210*2)*1e-18
        CspostL=N_inputs*(26*1.96+210*2.4)*1e-18
        CspostH=N_inputs*(26*1.96+210*2.4)*1e-18
        syn_input_exc=SynapseCircuitMultiwire(cell, N_inputs, N_excitatory, prez, postz, 4, 3, [Ctpre, Csr, CspreL, CspreH], [Ctpost, CspostL, CspostH], [], ([prez, ('xe',0.9), prez, prez],[postz, postz, postz]), syn_type='exc')
        #syn_input_exc=SynapseCircuit(cell, N_inputs, N_excitatory, trace_pre, trace_post, syn_type='exc', tracelim=1)
        syn_input_exc.W=winit
        ...
        syn_input_exc=SynapseCircuitMultiwire(cell, N_inputs, N_excitatory, prez, postz, 1, 1, [Cpre], [Cpost], [], ([('xe',0.8)],[postvr]), syn_type='exc')
    ...
    del net, excitatory, inhibitory, winit, syn_input_exc, syn_ei, syn_ie
...
import sys
flog=open('inf_7T1R_benchmark.log', 'a')
