...
from spikelearn import SpikingNet, HomeostasisLayer, SecondOrderLayer, PlasticSynapse, LambdaSynapse, OneToOneSynapse, SynapseCircuit, cell_Paiyu_chen_15, BaseSynapse, cell_Vteam
import numpy as np
...
flog=open(os.path.join('outputs_sweeps', '_'.join(argv[1:])), 'w')
...
for batch, label_batch in itertools.chain(*([zip(*batchify(train_images, train_labels)) for i in range(epoch)]+[((test_images, test_labels),)])):
    ...
    for sampleid, (sample, label) in enumerate(zip(batch, label_batch)):
        thistime=time.time()
        times.append(thistime-lasttime)
        lasttime=thistime
        freq=initial_freq
        outputcnt=0
...
times=times[1:]
print(np.average(times), np.std(times,ddof=1))
