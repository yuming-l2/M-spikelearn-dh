...
#     syn_L2.W=W2
    ...
    fname='model_data/1t1r_Paiyu_15.npz'
    # dep: e^30 per volt
    cell1=cell_data_driven_bp_T(W1, g1, lambda te:np.maximum(v1_p+np.log(te)*dv_ep, 0), lambda to:-np.maximum(v1_n+np.log(to)*dv_en, 0), 1e-9, 10e-9, fname, Vread=0.7)
    syn_L1=SynapseCircuitBP(cell1, N_input, 1500, trace_L1_pre, trace_L1_post, syn_type='exc', tracelim=1)
    syn_L1.W=W1
    cell2=cell_data_driven_bp_T(W2, g1, lambda te:np.maximum(v1_p+np.log(te)*dv_ep, 0), lambda to:-np.maximum(v1_n+np.log(to)*dv_en, 0), 1e-9, 10e-9, fname, Vread=0.7)
    syn_L2=SynapseCircuitBP(cell2, 1500, 10, trace_L2_pre, trace_L2_post, syn_type='exc', tracelim=1)
    syn_L2.W=W2
elif plastic_synapse_type=='1T1R_T_VTEAM':
    g1, v1_p, v1_n = plastic_synapse_params
    g1=float(g1)
    v1_p=float(v1_p)
    v1_n=float(v1_n)
    dv_ep=1/95
    dv_en=1/30
    fname='model_data/1t1r_Vteam.npz'
    # dep: e^30 per volt
    cell1=cell_data_driven_bp_T(W1, g1, lambda te:np.maximum(v1_p+np.log(te)*dv_ep, 0), lambda to:-np.maximum(v1_n+np.log(to)*dv_en, 0), 1e-9, 10e-9, fname, Vread=0.7)
    syn_L1=SynapseCircuitBP(cell1, N_input, 1500, trace_L1_pre, trace_L1_post, syn_type='exc', tracelim=1)
    syn_L1.W=W1
...
for iteration in range(3):
    ...
    for i in range(Ntrain):
        if i%BatchTrain==0 and (i>0 or iteration>0):
            ...
            stats1[:]=0
            stats2[:]=0
            if (len(accs)>5 and (np.array(accs[-3:])<0.2).all()) or (len(accs)>=3 and (np.array(accs[-2:])<0.15).all()):
                exit()
            if max(accs)>max(accs[-10:])+0.05:
                exit()
