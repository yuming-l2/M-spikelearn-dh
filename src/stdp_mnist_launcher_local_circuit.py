...
LOG_PATH='/local/data/yuming/stdp_mnist_deephyper3'
PREV_LOG_PATH='/local/data/yuming/stdp_mnist_deephyper2'
CHPT_PATH='/local/data/yuming/stdp_mnist_deephyper3/checkpoints'
DH_LOG_PATH='/local/data/yuming/stdp_mnist_deephyper3/dh_log_dir'
...
def launch(config):
    ...
    N_excitatory=prev_config['N_excitatory']
    tau_excitatory=prev_config['tau_excitatory']
    tau_inhibitory=prev_config['tau_inhibitory']
    refrac_e=prev_config['refrac_e']
    refrac_i=prev_config['refrac_i']
    tau_e2e=prev_config['tau_e2e']
    tau_e2i=prev_config['tau_e2i']
    tau_i2e=prev_config['tau_i2e']
    tau_i2i=prev_config['tau_i2i']
    tau_pre_trace=prev_config['tau_pre_trace']
    tau_post_trace=prev_config['tau_post_trace']
    w_ei=prev_config['w_ei']
    w_ie=prev_config['w_ie_over_ei']*w_ei
    tau_theta=prev_config['tau_theta']
    theta_delta=prev_config['theta_delta']
    theta_init=prev_config['theta_init']
    w_ee_colsum=prev_config['w_ee_colsum']
    input_steps=prev_config['input_steps']
    rest_steps=prev_config['rest_steps']
    initial_freq=prev_config['initial_freq']
    additional_freq=prev_config['additional_freq']
    g1=config['g1']
    v1_p=config['v1_p']
    v1_n=config['v1_n']
    dv_ep=config['dv_ep']
    ...
    chpt_name=hashlib.md5(('_'.join(map((lambda x:(('%.5e'%x) if type(x) is float else str(x))),[
        ...
        ]+list(plastic_synapse_params)))).encode('utf8')).hexdigest()+'_'+plastic_synapse_type+'_'+fname.replace('/','.')+'.chpt'
...
if True:
    ...
    df=pd.read_csv(os.path.join(PREV_LOG_PATH,'results.csv'))
    prev_config=df.iloc[df['objective'].argmax()]
