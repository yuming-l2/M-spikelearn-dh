...
# stats1=np.zeros(8)
# stats2=np.zeros(8)
Nsteps_total=3*Ntrain*(T//update_interval)
wtrace_size=100
W1_coordinates=np.random.choice(syn_L1.W.size, wtrace_size)
W2_coordinates=np.random.choice(syn_L2.W.size, wtrace_size)
W1_trace=np.empty((Nsteps_total,wtrace_size))
W2_trace=np.empty((Nsteps_total,wtrace_size))
wtrace_next=0
for iteration in range(3):
    np.random.seed(iteration+190)
    for i in range(Ntrain):
        ...
        for t in range(1,T):
            ...
            if (t)%update_interval==0:
                ...
                W1_trace[wtrace_next]=syn_L1.W.flatten()[W1_coordinates]
                W2_trace[wtrace_next]=syn_L2.W.flatten()[W2_coordinates]
                print(wtrace_next, np.abs(W1_trace[wtrace_next]-W1_trace[0]).max(), np.abs(W2_trace[wtrace_next]-W2_trace[0]).max())
                wtrace_next+=1
