...
k2_all={}
for line in lines:
    if '|' in line and line.startswith('src/stdp_7T1R_benchmark.py'):
        ...
        k1=(options.scale, options.data_set, options.stdp_type, options.stdp_step, options.layer_desc.replace(',','-'))
...
for seed in results.keys():
    ...
    for k2 in k2_list:
        print(','.join(map(str,k2)), *['']*5, sep=sep, end=sep, file=fout)
    print(file=fout)
    print(*['scale','dataset','stdp type','tau','layers'], sep=sep, end=sep, file=fout)
    for k2 in k2_list:
        print(*['read','update','pre','post','neuron','total'], sep=sep, end=sep, file=fout)
    print(file=fout)
    for k1 in sorted(results[seed].keys()):
        valdict=results[seed][k1]
        print(*k1, sep=sep, end=sep, file=fout)
        for k2 in k2_list:
            print(*valdict.get(k2,['#N/A']*6), sep=sep, end=sep, file=fout)
        print(file=fout)
