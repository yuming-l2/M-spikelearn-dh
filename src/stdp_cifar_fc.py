...
flog=open(os.path.join('outputs_cifar_sweeps', '_'.join([s.replace('/','-') for s in argv[1:]])), 'w')
...
if plastic_synapse_type=='ideal':
    ...
    syn_input_exc=PlasticSynapse(N_inputs, N_excitatory, winit, trace_pre, trace_post,
                            rule_params={'Ap':lr, 'An':lr_ratio*lr}, Wlim=1, syn_type='exc', tracelim=1)
else:
