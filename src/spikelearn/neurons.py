...
class LIFSubtract:
    """LIF neuron with v-=vth instead of v=0"""

    def __init__(self, N, tau=3, v0=1):
        self.N = N
        self.tau = tau
        self.a = 1/(1+np.exp(-tau))
        self.v0 = v0
        self.v = np.zeros(N)
        self.s = np.zeros(N)
    ...
    def __call__(self, *x):
        xtot = sum(x)
        self.v = self.v*self.a + xtot
        self.s = self.v>=self.v0
        self.v = self.v - self.s*self.v0
        return self.s
...
class TargetTimeLayer:

    def __init__(self, N, dt, vth0, eta, tobj):
        self.N = N
        self.dt=dt
        self.vth0=vth0
        self.eta=eta
        self.tobj=tobj
        self.vth = np.empty(self.N)
        self.vth[:]=self.vth0
    ...
    def __call__(self, *x):
        ...
        if self.spiked:
            ...
            s_all = hard(self.v-self.vth)
            if s_all.any():
                idx=np.random.choice(np.where(s_all)[0])
                self.s[idx]=1
                self.v[idx]=0
                self.vth[self.s]+=self.eta*(self.tobj-self.now+1)
                self.spiked=True
        return self.s
    ...
    def reset(self):
        self.v = np.zeros(self.N)
        self.s = np.zeros(self.N, dtype=np.int8)
        self.spiked=False
        self.now=0
