...
datafname='model_data/1t1r_Vteam.npz'
datafile=np.load(datafname)
G1list=[5e-06, 7e-06, 1e-05, 3e-05, 0.0001, 0.0003]#  [3e-6, 1e-5]#[3e-5, 1e-4, 3e-4]
...
for G1 in G1list:
    for centerW in centerWlist:
        lramps[(G1, centerW)]=[]
schedf=open('stdp_mnist_1t1r_adapt_sched.txt','r')
for line in schedf:
    G1, centerW, Alrp, Alrn = map(float,line.split())
    lramps[(G1, centerW)].append((Alrp, Alrn))
...
for G1 in G1list:
    for centerW in centerWlist:
        ...
        for Alrp, Alrn in lramps[(G1, centerW_ori)]:
...
def work(G1, pot_vbase, dep_vbase):
    ret=os.system(f'srun -p fast-long -c 1 -u python3 src/stdp_mnist.py 400 10000 2 data-driven {datafname} {G1} {pot_vbase} {dep_vbase} {dv_ep} {dv_en}')
    if ret:
